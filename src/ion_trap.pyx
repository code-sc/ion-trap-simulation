...
def total_energy(vector[double] x):
    cdef n = x.size()
    return ion_trap_lib.total_energy(n, x)
...
def neg_grad_energy(vector[double] x,
                    ...
                    double dx=1e-9):
    cdef n = x.size()
    return ion_trap_lib.neg_grad_energy(n, x, k, dx)
